        ...
        private async Task InitializeAllActiveAccountsAsync()
        {
            ...
            try
            {
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(account.Affiliated_IP))
                        {
                            _logger.LogInformation($"Sending START command for account {account.Id} with IP {account.Affiliated_IP}");

                            var result = await _pythonApiService.StartAccountAsync(account.Id.ToString(), account.Affiliated_IP);
                        ...
                        {
                            _logger.LogWarning($"Account {account.Id} has no affiliated IP, skipping");
                        }
                    }
                }
