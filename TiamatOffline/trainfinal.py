...
SPECIFIED_PARAMS = {
    'n_estimators': 600,
    'learning_rate': 0.022144186881709514,
    'max_depth': 7,
    'num_leaves': 42,
    'min_child_samples': 88,
    'subsample': 0.6085859292702106,
    'colsample_bytree': 0.659524526531164,
    'reg_alpha': 0.40769740208782235,
    'reg_lambda': 0.3695570487202825,
    'path_smooth': 6.649332346386287,
    'subsample_freq': 3,
    'min_split_gain': 0.01,
    'max_bin': 255,
    'boosting_type': 'gbdt',
...
def prune_features(df_train, model=None, feature_imp=None, prune_percent=20):
    ...
    logger.info("Full list of features being used for the model:")
...
def train_final_model(df_all, bin_edges, features_to_use, timeframe, params):
    logger.info(f"Training FINAL model on ALL data (2018-2024): {len(df_all)} rows")
    logger.info(f"Period: [{df_all['timestamp'].min()} -> {df_all['timestamp'].max()}]")
    
    df_all_labeled = create_labels(df_all, timeframe, bin_edges)
    unique_bins = sorted(df_all_labeled["movement_bin"].unique())
    num_classes = len(unique_bins)
    
    split_idx = int(len(df_all_labeled) * 0.9)
    df_train_split = df_all_labeled.iloc[:split_idx]
    df_val_split = df_all_labeled.iloc[split_idx:]
    ...
    logger.info(f"Final model: using {len(X_train)} samples for training, {len(X_val)} for validation")
...
def run_backtest(df, strategy_params=None, save_plot=False, plot_filename=None, html_filename=None):
    df_bt = df.copy()
    df_bt.rename(
        columns={
            'open': 'Open',
    ...
    if save_plot:
        if html_filename:
            fig = bt.plot(filename=html_filename, open_browser=False)
            logger.info(f"Backtest HTML saved to {html_filename}")
        elif plot_filename:
...
def log_detailed_results(stats, period_name):
    logger.info(f"===== {period_name} BACKTEST RESULTS =====")
    ...
    logger.info(f"Expected Return: {stats['Expectancy [%]']:.2f}%")
...
def run_single_trial():
    ...
    logger.info(f"Using specified parameters: {params}")
    
    logger.info("=== STAGE 1: Training model on 2018-2022 data ===")
    
    train_sample = df_train_global.sample(frac=0.3, random_state=RANDOM_SEED)
    val_sample = df_val_global.sample(frac=0.3, random_state=RANDOM_SEED)
    ...
    logger.info("Running backtest on 2023 validation data...")
    stats_val, bt_val = run_backtest(
        df_val_signals, 
        save_plot=True, 
        html_filename=os.path.join(backtest_results_dir, "validation_backtest_2023.html")
    )
    
    log_detailed_results(stats_val, "2023 VALIDATION")
    
    logger.info("Generating probability difference plot for 2023 validation...")
    plot_prob_diff_with_trades(
        df_val_signals, 
        bt_val, 
        "Validation 2023",
        os.path.join(charts_dir, "val_prob_diff_2023.png")
    )
    
    val_model_path = os.path.join(models_dir, "val_model_2018_2022.pkl")
    save_model_bundle(
        model=val_model,
        bin_edges=bin_edges,
        timeframe=timeframe,
        ...
        output_path=val_model_path
    )
    
    logger.info("=== STAGE 2: Training incremental model on 2018-2023 data ===")
    ...
    logger.info("Running backtest on 2024 test data...")
    stats_test, bt_test = run_backtest(
        df_test_signals, 
        save_plot=True, 
        html_filename=os.path.join(backtest_results_dir, "test_backtest_2024.html")
    )
    
    log_detailed_results(stats_test, "2024 TEST")
    
    logger.info("Generating probability difference plot for 2024 test...")
    plot_prob_diff_with_trades(
        df_test_signals, 
        bt_test, 
        "Test 2024",
        os.path.join(charts_dir, "test_prob_diff_2024.png")
    )
    
    incr_model_path = os.path.join(models_dir, "incremental_model_2018_2023.pkl")
    save_model_bundle(
        ...
        output_path=incr_model_path
    )
    
    logger.info("=== STAGE 3: Training FINAL model on ALL data (2018-2024) ===")
    ...
    final_model_path = os.path.join(models_dir, "FINAL_MODEL_2018_2024_FOR_LIVE.pkl")
    save_model_bundle(
        model=final_model,
        bin_edges=bin_edges,
        timeframe=timeframe,
    ...
    logger.info("\n\n======= RESULTS SUMMARY =======")
    logger.info(f"2023 Validation: Sharpe={stats_val['Sharpe Ratio']:.2f}, Return={stats_val['Return [%]']:.2f}%, " 
                f"MaxDD={stats_val['Max. Drawdown [%]']:.2f}%, Trades={stats_val['# Trades']}")
    logger.info(f"2024 Test: Sharpe={stats_test['Sharpe Ratio']:.2f}, Return={stats_test['Return [%]']:.2f}%, "
                f"MaxDD={stats_test['Max. Drawdown [%]']:.2f}%, Trades={stats_test['# Trades']}")
    logger.info("\nModels saved:")
    logger.info(f"Validation model (2018-2022): {val_model_path}")
    logger.info(f"Incremental model (2018-2023): {incr_model_path}")
    logger.info(f"FINAL MODEL FOR LIVE TRADING (2018-2024): {final_model_path}")
    logger.info(f"\nBacktest HTML results saved to {backtest_results_dir}/")
...
if __name__ == "__main__":
    final_features = run_single_trial()
